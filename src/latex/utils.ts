import { INode, ITag } from "../html5parser/types"
...
export function renderLabel(rendered:string[],node:ITag){
    if (node.attributeMap.label)
        rendered.push(latexCommand("label",[node.name,"-",node.attributeMap.label].join("")))
}